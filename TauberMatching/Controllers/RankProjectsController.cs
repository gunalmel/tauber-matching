    ...
    public class RankProjectsController : Controller
    {
        ...
        public JsonResult SubmitPreferences(StudentPreferencesDto preferencesDto)
        {
            ...
            if (preferencesDto.StudentFeedback != null)
            {
                ...
                {
                    // If a project is assigned multiple feedback scores find it do not include second score
                    if(feedbacks.Count>0&&feedbacks.Any(f=>f.Project.Id==sfDto.ProjectId))
                        continue;
